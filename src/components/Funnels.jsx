  ...
  return (
    <section style={{ background:'#04030F', position:'relative', overflow:'hidden', padding:'5rem 0' }}>
      <style>{`
        ...
        .fn-step { display:flex; align-items:center; gap:.75rem; color:#94A3B8; font-size:.82rem; font-weight:500; }
        @media(max-width:860px){ .fn-grid{grid-template-columns:1fr;} }
      `}</style>

      <div style={{ position:'absolute', inset:0, backgroundImage:'radial-gradient(rgba(124,58,237,.07) 1px,transparent 1px)', backgroundSize:'32px 32px', pointerEvents:'none' }}/>
      <motion.div animate={{ scale:[1,1.1,1], opacity:[.1,.18,.1] }} transition={{ duration:11, repeat:Infinity }}
        style={{ position:'absolute', left:'-8%', bottom:'10%', width:'500px', height:'500px', borderRadius:'50%', background:'radial-gradient(circle,rgba(124,58,237,.25) 0%,transparent 65%)', pointerEvents:'none' }}/>
      ...
      <div style={{ position:'relative', zIndex:1 }}>
        ...
        <div style={{ textAlign:'center', marginBottom:'3rem', padding:'0 2rem' }}>
          <motion.div initial={{ opacity:0, y:12 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }}
            ...
            <span style={{ fontSize:'.72rem', fontWeight:800, color:'#C4B5FD', letterSpacing:'.07em', textTransform:'uppercase' }}>Education & Strategy</span>
          </motion.div>
          ...
          <p style={{ color:'#64748B', fontSize:'.92rem', maxWidth:'520px', margin:'0 auto', lineHeight:1.6 }}>
        ...
        <div className="fn-grid">
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'.65rem' }}>
            {FUNNELS.map((fn, i) => {
              ...
              return (
                <motion.button key={fn.title} className="fn-tab" onClick={() => setActive(i)}
                  whileHover={{ x:4 }}
                  style={{ background: isA ? fn.bg : 'rgba(255,255,255,.02)', border:`1px solid ${isA ? fn.border : 'rgba(255,255,255,.06)'}`,
                    boxShadow: isA ? `0 0 24px -8px ${fn.glow}` : 'none' }}>
                  <div style={{ width:'36px', height:'36px', borderRadius:'10px', background: isA ? `${fn.color}20` : 'rgba(255,255,255,.04)', border:`1px solid ${isA ? fn.border : 'rgba(255,255,255,.06)'}`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, transition:'all .22s' }}>
                    <fn.Icon size={16} color={isA ? fn.color : '#4B5563'} strokeWidth={2}/>
                  </div>
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:'.88rem', fontWeight: isA ? 800 : 600, color: isA ? 'white' : '#6B7280', lineHeight:1.3, transition:'color .22s' }}>{fn.title}</div>
                    <div style={{ fontSize:'.68rem', color: isA ? fn.color : '#374151', fontWeight:600, marginTop:'2px', transition:'color .22s' }}>{fn.bestFor}</div>
                  </div>
            ...
            {/* Result badge */}
            <div style={{ padding:'.85rem 1.1rem', background:'rgba(34,197,94,.05)', border:'1px solid rgba(34,197,94,.12)', borderRadius:'12px', marginTop:'.25rem' }}>
              <div style={{ fontSize:'.72rem', color:'#86EFAC', fontWeight:700, marginBottom:'.2rem' }}>
                Avg. result with this system:
              </div>
              <div style={{ fontSize:'.95rem', color:'#22C55E', fontWeight:900 }}>{f.result}</div>
            </div>
          </div>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={active}
              ...
              style={{ background:`linear-gradient(145deg, ${f.bg}, rgba(255,255,255,.01))`, border:`1px solid ${f.border}`, borderRadius:'20px', padding:'2rem', position:'relative', overflow:'hidden', boxShadow:`0 0 50px -15px ${f.glow}` }}>
              ...
              <h3 style={{ color:'white', fontWeight:900, fontSize:'1.3rem', letterSpacing:'-.03em', lineHeight:1.15, margin:'0 0 .6rem' }}>{f.title}</h3>
              <p style={{ color:'#94A3B8', fontSize:'.85rem', lineHeight:1.6, margin:'0 0 1.25rem' }}>{f.desc}</p>

              {/* Steps */}
              <div style={{ display:'flex', flexDirection:'column', gap:'.55rem', marginBottom:'1.5rem' }}>
                {f.steps.map((step, si) => (
                  <div key={step} className="fn-step">
                    <div style={{ width:'20px', height:'20px', borderRadius:'50%', background:`${f.color}20`, border:`1px solid ${f.border}`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                      <span style={{ fontSize:'.58rem', fontWeight:900, color:f.color }}>{si+1}</span>
                    </div>
                    <CheckCircle size={12} color={f.color} style={{ flexShrink:0 }}/>
                    {step}
                  </div>
                ))}
              </div>

    ...
    <div className="dark-page" style={{ background: '#F8FAFC', color: '#0F172A', paddingTop: 80, minHeight: '100vh' }}>
      ...
      <section style={{ padding: '6rem 2rem 5rem', textAlign: 'center', maxWidth: 820, margin: '0 auto' }}>
        <motion.div {...fin}>
          ...
          <p style={{ fontSize: '1.15rem', color: '#475569', lineHeight: 1.75, maxWidth: 600, margin: '0 auto 1rem' }}>
      ...
      {/* ── OPEN ROLES ── */}
      <section style={{ borderTop: '1px solid rgba(0,0,0,0.08)', padding: '6rem 2rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1rem' }}>
            {ROLES.map((role, i) => (
              <motion.div key={role.title} {...fin} transition={{ delay: i * 0.07 }}
                ...
                style={{ padding: '1.75rem', background: activeRole === i ? `${role.color}0d` : '#ffffff', border: `1px solid ${activeRole === i ? role.color + '40' : 'rgba(0,0,0,0.08)'}`, borderRadius: 20, cursor: 'pointer', transition: 'all 0.2s', boxShadow: activeRole === i ? `0 0 30px -8px ${role.color}40` : '0 1px 4px rgba(0,0,0,0.06)' }}>
                ...
                <div style={{ fontWeight: 800, fontSize: '1.05rem', color: '#0F172A', marginBottom: '0.3rem' }}>{role.title}</div>
                <div style={{ fontSize: '0.75rem', color: '#64748B', marginBottom: '0.75rem' }}>{role.mode}</div>
                <AnimatePresence>
                  {activeRole === i && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}>
                      <p style={{ color: '#475569', fontSize: '0.88rem', lineHeight: 1.6, marginBottom: '1.25rem' }}>{role.desc}</p>
      ...
      {/* ── IS THIS YOU? ── */}
      <section style={{ padding: '6rem 2rem', background: '#F1F5F9', borderTop: '1px solid rgba(0,0,0,0.08)' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {TRAITS.map(t => (
                <div key={t} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem' }}>
                  ...
                  <span style={{ fontSize: '0.92rem', color: '#334155', lineHeight: 1.5 }}>{t}</span>
                </div>
              ))}
            </div>
          </motion.div>
          <motion.div {...fin} transition={{ delay: 0.1 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {NOT_FOR_YOU.map(t => (
                <div key={t} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem', padding: '0.75rem 1rem', background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.15)', borderRadius: 12 }}>
                  <X size={15} color="#EF4444" style={{ flexShrink: 0, marginTop: 1 }} />
                  <span style={{ fontSize: '0.88rem', color: '#475569', lineHeight: 1.5 }}>{t}</span>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.25rem' }}>
            {CULTURE.map((c, i) => (
              <motion.div key={c.title} {...fin} transition={{ delay: i * 0.08 }}
                style={{ padding: '2rem', background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 20, display: 'flex', gap: '1rem', alignItems: 'flex-start', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
                ...
                <div>
                  <div style={{ fontWeight: 800, fontSize: '0.95rem', color: '#0F172A', marginBottom: '0.4rem' }}>{c.title}</div>
      ...
      {/* ── PARTNERS ── */}
      <section style={{ padding: '5rem 2rem 6rem', background: '#F1F5F9', borderTop: '1px solid rgba(0,0,0,0.08)' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
            {PARTNERS.map((p, i) => (
              <motion.div key={p.type} {...fin} transition={{ delay: i * 0.08 }}
                style={{ padding: '1.75rem', background: '#FFFFFF', border: `1px solid ${p.color}25`, borderRadius: 18, borderTop: `2px solid ${p.color}`, boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
                <div style={{ fontWeight: 800, fontSize: '0.95rem', color: '#0F172A', marginBottom: '0.5rem' }}>{p.type}</div>
          ...
          <motion.div {...fin} style={{ textAlign: 'center', marginTop: '2.5rem' }}>
            <a href={`${WA}?text=Hi! I want to explore a partnership with Creativals.`} target="_blank" rel="noreferrer"
              style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.8rem 1.75rem', background: '#ffffff', border: '1px solid rgba(0,0,0,0.12)', borderRadius: 12, color: '#334155', fontWeight: 700, fontSize: '0.9rem', textDecoration: 'none', transition: 'all 0.2s' }}>
